...
public class TimeOutPanel : BasePanel
{
    public override string PanelName => "TimeOutPanel";
    ...
    private void OnBackButtonClick()
    {
        ...
        // 关闭 TimeOutPanel 和 StagePanel
        if (UIManager.Instance != null)
        {
            UIManager.Instance.ClosePanel("TimeOutPanel");
            UIManager.Instance.ClosePanel("StagePanel");
        }
    }

    /// <summary>
    /// 重试按钮点击事件（重置时间到关卡默认时间）
    /// </summary>
    private void OnRetryButtonClick()
    {
        PlayButtonSound(false); // 播放普通按钮音效
        Debug.Log("[TimeOutPanel] 重试：重置时间到关卡默认时间");
        
        // 重置时间到默认值（关卡默认时间）
        if (StageManager.Instance != null)
        {
            StageManager.Instance.ResetTimer();
        }
